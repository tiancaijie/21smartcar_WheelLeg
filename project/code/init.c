...
void Init_All(void)
{
    ...
    /* ----IO口初始化---- */
    gpio_init(Buzzer_Pin, GPO, 0, GPO_PUSH_PULL);                 //蜂鸣�?
    gpio_init(Switch_Pin, GPI, 0, GPI_PULL_DOWN);                 //使能开�?
    /* ----舵机初�?�化---- */
    pwm_init(Left_FLeg_PWM1, 300, LeftF_Init);                    //�?
    pwm_init(Left_BLeg_PWM2, 300, LeftB_Init);
    pwm_init(Right_FLeg_PWM1, 300, RightF_Init);                  //�?
